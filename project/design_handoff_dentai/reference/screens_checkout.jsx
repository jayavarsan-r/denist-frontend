...
function CheckoutScreen({ entryId }) {
  const app = useApp();
  const entry = app.queue.find(e => e.id === entryId);
  ...
  const complete = () => {
    app.checkout(entry.id, { patientName: p.name, procedure: `${c.procedure}${c.tooth ? ' · Tooth ' + c.tooth : ''}`, amount: paid });
    app.showToast('Checked out · ' + formatCurrency(paid) + ' collected');
    app.goBack();
  };
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      <NavBar title="Checkout" onBack={app.goBack} right={<StatusChip status={status} />} />
      <div className="scroll" style={{ flex: 1, padding: '16px 20px 28px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
          ...
          <button onClick={() => app.openPatient(p.id)} style={{ color: 'var(--blue)', fontSize: 14, fontWeight: 500 }}>Profile</button>
        </div>
        ...
        <div className="card" style={{ padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <div style={{ marginLeft: 'auto' }}>{(() => { const o = DATA.CONSULT_OUTCOMES.find(x => x.id === entry.outcome); return o ? <Chip label={o.label} tone={o.tone} /> : null; })()}</div>
          </div>
        ...
        {c.medicines && c.medicines.length > 0 && (
          <>
            <SectionHeader right={<div style={{ display: 'flex', gap: 14 }}><button onClick={() => app.showToast('Generating PDF…')} style={{ color: 'var(--blue)', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}><Icon name="printer" size={14} color="var(--blue)" />PDF</button><button onClick={() => app.showToast('Shared via WhatsApp')} style={{ color: 'var(--blue)', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}><Icon name="share" size={14} color="var(--blue)" />Share</button></div>}>Prescription</SectionHeader>
            <div className="card" style={{ overflow: 'hidden', marginBottom: 16 }}>
              {/* column header */}
        ...
        <button onClick={app.goBack} style={{ width: '100%', textAlign: 'center', color: 'var(--text-secondary)', fontSize: 15, fontWeight: 500, padding: '14px 0 2px' }}>Save & review later</button>
      </div>
    </div>
  );
}

Object.assign(window, { CheckoutScreen, MealTiming });
