/* DentWay — Lab order card, Lab screen, Finance screen */

function daysBetween(a, b) { return Math.round((parseDate(b) - parseDate(a)) / 86400000); }

function LabOrderCard({ order, app, compact }) {
  const margin = order.chargedToPatient - order.costToClinic;
  const overdue = order.status === 'sent' && order.expectedReturnDate < DATA.TODAY;
  const rem = daysBetween(DATA.TODAY, order.expectedReturnDate);
  ...
  return (
    <button onClick={() => app.openSheet('labDetail', { id: order.id })} className="card tap" style={{ width: '100%', padding: 16, marginBottom: 10, textAlign: 'left', display: 'block', borderLeft: overdue ? '3px solid var(--amber)' : 'none' }}>
      ...
      {order.status === 'sent' && (
        <div style={{ display: 'flex', gap: 14, marginTop: 12 }}>
          <button onClick={(e) => { e.stopPropagation(); app.markLabReceived(order.id); }} style={{ height: 36, padding: '0 16px', borderRadius: 12, border: '1px solid var(--border)', fontSize: 14, fontWeight: 600, background: '#fff' }}>Mark received</button>
...
function LabScreen() {
  ...
  const orders = app.labOrders.filter(o => filter === 'All' || o.status === filter);
  const activeCount = app.labOrders.filter(o => o.status === 'sent' || o.status === 'pending').length;
  const owed = app.labOrders.filter(o => o.status !== 'completed').reduce((s, o) => s + o.costToClinic, 0);
  const margin = app.labOrders.reduce((s, o) => s + (o.chargedToPatient - o.costToClinic), 0);
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <NavBar title="Lab orders" onBack={app.goBack} right={<button onClick={() => app.openSheet('newLab', {})} style={{ color: 'var(--accent)', display: 'flex' }}><Icon name="plus" size={24} stroke={2.2} /></button>} />
      <div className="scroll" style={{ flex: 1, padding: '14px 0 24px' }}>
        ...
        <div style={{ padding: '0 20px' }}>
          {orders.length === 0 ? <div className="card"><EmptyState icon="flask" title="No lab orders" hint="Tap + to create one" /></div> : orders.map(o => <LabOrderCard key={o.id} order={o} app={app} />)}
        </div>
      </div>
    </div>
  );
...
function FinanceScreen() {
  ...
  const collectedToday = app.clinicAccounts.filter(a => a.type === 'income' && a.date === today).reduce((s, a) => s + a.amount, 0);
  const pendingBills = app.bills.filter(b => b.outstanding > 0)
  ...
  const labDue = app.labOrders.filter(o => o.status !== 'completed').reduce((s, o) => s + o.costToClinic, 0);
  const recent = [...app.clinicAccounts].sort((a, b) => b.date.localeCompare(a.date));
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--surface)' }}>
      <div className="scroll" style={{ flex: 1 }}>
        ...
        <div style={{ padding: '58px 22px 4px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => app.openSheet('addEntry')} style={{ width: 42, height: 42, borderRadius: '50%', background: 'rgba(60,60,67,0.07)', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="plus" size={22} stroke={2.2} /></button>
        </div>
          ...
          {pendingBills.length === 0 ? (
            ...
            <div>
              {pendingBills.map((b, i) => (
                <button key={b.id} onClick={() => app.openPatient(b.patientId, 'Billing')} className="rowtap" style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderTop: i ? '1px solid var(--border-light)' : 'none', textAlign: 'left' }}>
        ...
        <div style={{ padding: '28px 22px 0' }}>
          <Eyebrow action={<button onClick={() => app.openLab()} style={{ fontSize: 13, fontWeight: 600, color: 'var(--blue)' }}>All orders</button>}>Lab payments · {app.labOrders.length}</Eyebrow>
          <div>
            {app.labOrders.map((o, i) => {
              const st = labState(o);
              return (
                <button key={o.id} onClick={() => app.openSheet('labDetail', { id: o.id })} className="rowtap" style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '13px 0', borderTop: i ? '1px solid var(--border-light)' : 'none', textAlign: 'left' }}>
...
Object.assign(window, { LabOrderCard, LabScreen, FinanceScreen, daysBetween });
