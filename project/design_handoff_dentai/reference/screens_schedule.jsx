...
function ApptBlock({ v, app, colW, onDragMove, onOpen, dragging }) {
  const p = app.patients.find(x => x.id === v.patientId);
...
function WeekView({ app }) {
  ...
  const word = { confirmed: 'Confirmed', arrived: 'In chair', done: 'Done', no_show: 'No-show' };
    ...
    <div className="scroll" style={{ flex: 1, background: 'var(--surface)' }}>
      <div style={{ padding: '4px 22px 28px' }}>
        {days.map((d, di) => {
          ...
          const dayVisits = app.visits.filter(v => v.date === iso).sort((a, b) => a.startTime.localeCompare(b.startTime));
          return (
              ...
              {dayVisits.length === 0 ? (
                ...
                <div>
                  {dayVisits.map((v, i) => {
                    ...
                    return (
                      <button key={v.id} onClick={() => app.openSheet('apptPeek', { id: v.id })} className="rowtap" style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '11px 0', borderTop: i ? '1px solid var(--border-light)' : 'none', textAlign: 'left', opacity: isPast ? 0.62 : 1 }}>
    ...
    <div className="scroll" style={{ flex: 1, position: 'relative' }}>
      <div style={{ position: 'relative', height: slots * SLOT_PX, marginLeft: TIME_COL, marginRight: 8 }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, width: colW, height: '100%' }}>
          {dayVisits.map(v => <ApptBlock key={v.id} v={v} app={app} colW={colW} onDragMove={() => {}} onOpen={(vv) => app.openSheet('apptPeek', { id: vv.id })} dragging={false} />)}
        </div>
        {nowFrac != null && (
...
function MonthView({ app }) {
  const base = parseDate(DATA.TODAY);
  ...
  return (
    <div className="scroll" style={{ flex: 1, padding: '8px 12px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gridAutoRows: 64, gap: 2 }}>
        {cells.map((d, i) => {
          const iso = toISO(d); const inMonth = d.getMonth() === base.getMonth(); const isToday = iso === DATA.TODAY;
          const dayV = app.visits.filter(v => v.date === iso);
          return (
            <button key={i} onClick={() => app.setScheduleView('Day')} style={{ borderRadius: 8, padding: 4, textAlign: 'left', background: 'transparent', opacity: inMonth ? 1 : 0.32, display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                {dayV.slice(0, 3).map(v => { const proc = app.procedures.find(x => x.id === v.procedureId); return <div key={v.id} style={{ width: 5, height: 5, borderRadius: '50%', background: getProcedureColor(proc ? proc.type : 'Other').dot }} />; })}
...
function ScheduleScreen() {
  const app = useApp();
  const view = app.scheduleView;
  const hasAny = app.visits.length > 0;
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ flexShrink: 0, background: 'var(--surface)', borderBottom: '1px solid var(--border-light)' }}>
        <div style={{ padding: '58px 20px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => app.openSheet('newVisit', {})} style={{ color: 'var(--accent)', display: 'flex' }}><Icon name="plus" size={26} stroke={2.4} /></button>
        </div>
        <div style={{ padding: '0 20px 12px' }}>
          <Segmented options={['Day', 'Week', 'Month']} value={view} onChange={app.setScheduleView} />
        </div>
      </div>
      {!hasAny ? <EmptyState title="No appointments" hint="Tap + to schedule" /> :
        view === 'Week' ? <WeekView app={app} /> : view === 'Day' ? <DayView app={app} /> : <MonthView app={app} />}
    </div>
  );
}

Object.assign(window, { ScheduleScreen, WeekView, DayView, MonthView });
